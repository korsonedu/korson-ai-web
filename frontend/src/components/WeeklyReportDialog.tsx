...
export const WeeklyReportDialog: React.FC = () => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { if(!open) handleClose(); }}>
      <DialogContent className="w-[96vw] sm:max-w-[920px] rounded-[2rem] p-0 border-none bg-white shadow-2xl overflow-hidden max-h-[88vh]">
        <div className="grid md:grid-cols-[340px_1fr] h-full">
          ...
          <div className="p-6 md:p-8 space-y-5 bg-slate-50/50 overflow-y-auto">
            <div className="p-6 bg-white rounded-[1.6rem] border border-black/[0.03] shadow-sm">
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  <div className="h-10 w-10 rounded-2xl bg-slate-900 flex items-center justify-center shrink-0 shadow-lg shadow-indigo-200/20">
                ...
                <div className="grid grid-cols-2 gap-3 md:min-w-[220px]">
                  <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Current ELO</p>
                    <p className="text-lg font-black text-slate-900 tabular-nums">{report.current_elo}</p>
                  </div>
                  <div className="rounded-2xl bg-slate-50 border border-slate-100 p-3 text-center">
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Reviews</p>
                    <p className="text-lg font-black text-slate-900 tabular-nums">{report.week_reviews}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="p-5 bg-white rounded-[1.4rem] border border-black/[0.03] shadow-sm space-y-3">
            ...
            <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Weekly Insight</span>
              </div>
              <Button onClick={handleClose} className="h-11 rounded-xl bg-slate-900 text-white font-black shadow-lg hover:bg-slate-800 transition-all active:scale-95 tracking-widest text-[11px] px-5">
                开启新一周 <ChevronRight className="ml-1.5 w-4 h-4" />
              </Button>
            </div>
          </div>
        </div>
        <div className="md:hidden p-4 bg-slate-50 border-t border-slate-100">
          <Button onClick={handleClose} className="w-full h-12 rounded-xl bg-slate-900 text-white font-black shadow-lg hover:bg-slate-800 transition-all active:scale-95 tracking-widest text-[11px]">
